...
def write_params_O32O33(params):
    ...
    ylabels = ['teff', 'vsini', 'rv', 'airmass', 'pwv', 'veiling', 'lsf', 'noise']
    for order in orders:
    ...
    veiling_params = []
    for order in orders:
        model_veiling = smart.Model(teff=mcmc[0, 0], logg=4., order=str(order), modelset='phoenix-aces-agss-cond-2011', instrument='nirspec')
        veiling_params.append(mcmc[5, 0] / np.median(model_veiling.flux))
    ...
    for i, order in enumerate(orders):
        model, model_notel = smart.makeModel(mcmc[0, 0], order=str(order), data=sci_specs[i], logg=4.0, vsini=mcmc[1, 0], rv=mcmc[2, 0], airmass=mcmc[3, 0], pwv=mcmc[4, 0], veiling=mcmc[5, 0], lsf=mcmc[6, 0], wave_offset=mcmc[2*i - 2*np.size(orders), 0], flux_offset=mcmc[2*i - 2*np.size(orders) + 1, 0], z=0, modelset='phoenix-aces-agss-cond-2011', output_stellar_model=True)
        models.append(model)
    ...
    result = {
        ...
        'wave_offset_O32':      mcmc[8, :], 
        'flux_offset_O32':      mcmc[9, :], 
        'wave_offset_O33':      mcmc[10, :], 
        'flux_offset_O33':      mcmc[11, :], 
...
if __name__=='__main__':
    ...
    for i in range(len(names)):
        ...
        write_params_O32O33(params=params)
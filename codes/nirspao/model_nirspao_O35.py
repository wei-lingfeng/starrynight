...
def median_combine_teff(infos, orders=[35], Multiprocess=True, MCMC=True, Finetune=True, nwalkers=100, steps=500):
    ...
    itime = sci_specs[0].header['ITIME']
    ...
    # Modify params
    pos = [np.append([
        np.random.uniform(4000, 7000),  # teff
    ...
    result = {
        ...
        'veiling_param_O35':    veiling_params[0],
        'lsf':                  mcmc.lsf, 
        'noise':                mcmc.noise,
        'model_dip_O35':        model_dips[0],
        'model_std_O35':        model_stds[0],
        'wave_offset_O35':      mcmc.wave_offset_O35, 
        'flux_offset_O35':      mcmc.flux_offset_O35, 
        'snr_O35':              np.median(sci_specs[0].flux/sci_specs[0].noise),
    }
    ...
    for order in orders:
        model_veiling = smart.Model(teff=mcmc.teff[0], logg=4., order=str(order), modelset='phoenix-aces-agss-cond-2011', instrument='nirspec')
        veiling_params.append(mcmc.veiling / np.median(model_veiling.flux))
...
def lnprior(theta):
    ...
    # Modify Orders
    teff, vsini, rv, airmass, pwv, veiling, lsf, noise, wave_offset1, flux_offset1 = theta
...
def lnlike(theta, sci_specs, orders):
    ...
    # Modify Orders
    wave_offsets = [wave_offset1]
    flux_offsets = [flux_offset1]
...
if __name__=='__main__':
    ...
    dates = [(20, 1, 21)]
    names = ['504']
    sci_frames = [[43, 44, 45, 46]]
    tel_frames = [[51, 52, 52, 51]]
    ...
    for i in range(len(names)):
        ...
        result = median_combine_teff(infos=infos, MCMC=True, Finetune=False, Multiprocess=True, steps=500)
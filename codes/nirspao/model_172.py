...
def plot_spectrum(sci_spec, result, spec_lines=None, save_path=None, mark_CO=True, show_figure=False):
    ...
    model, model_notel = smart.makeModel(result['teff'][0], data=sci_spec, order=order, logg=result['logg'], vsini=result['vsini'][0], rv=result['rv'][0], airmass=result['airmass'][0], pwv=result['pwv'][0], veiling=33927.89, lsf=result['lsf'][0], wave_offset=result[f'wave_offset_O{order}'][0], z=0, modelset='phoenix-aces-agss-cond-2011', output_stellar_model=True)
...
def lnprior(theta, orders, limits):
    ...
    teff, vsini, rv, airmass, pwv, lsf, noise = theta[:-len(orders)]
...
def lnlike(theta, sci_specs, orders, logg):
    ...
    for sci_spec, wave_offset, order in zip(sci_specs, wave_offsets, orders):
        model = smart.makeModel(teff, logg=logg, vsini=vsini, rv=rv, airmass=airmass, pwv=pwv, veiling=33927.89, lsf=lsf, z=0, wave_offset=wave_offset, order=order, data=sci_spec, modelset='phoenix-aces-agss-cond-2011')
...
def model_nirspao(infos:dict, orders=[32, 33], initial_mcmc=True, finetune=True, finetune_mcmc=True, multiprocess=True, nwalkers=100, steps=300, **kwargs):
    ...
    # modify parameters
    params = ['teff', 'vsini', 'rv', 'airmass', 'pwv', 'lsf', 'noise']
    for order in orders:
    ...
    for i, order in enumerate(orders):
        model, model_notel = smart.makeModel(mcmc.teff[0], data=sci_specs[i], order=order, logg=logg, vsini=mcmc.vsini[0], rv=mcmc.rv[0], airmass=mcmc.airmass[0], pwv=mcmc.pwv[0], veiling=veiling, lsf=mcmc.lsf[0], wave_offset=mcmc.loc[0, f'wave_offset_O{order}'], z=0, modelset='phoenix-aces-agss-cond-2011', output_stellar_model=True)
        ...
        other_params[f'veiling_param_O{order}'] = veiling / np.median(model_veiling.flux)

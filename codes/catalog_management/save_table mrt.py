...
# save mrt
# obs_table = QTable()
# obs_table['HC2000 ID'] = MaskedColumn(obs_df['HC2000 ID'], format=str, description='Identifier in [HC2000], Hillenbrand & Carpenter (2000) [2000ApJ...540..236H]')
# obs_table['Obs Time'] = MaskedColumn(obs_df['Obs. Date'], format=str, description='Observation Date')
# obs_table['No. of Frames'] = MaskedColumn(obs_df['No. of Frames'], description='Number of Frames')
# obs_table['Int. Time'] = MaskedColumn(obs_df['Int. Time'], unit=u.s, description='Integration Time of each frame')
...
# mrt table
result_mrt = Table()
result_mrt['HC2000']          = MaskedColumn(ma.array([f"HC2000 {result.HC2000[i]:.0f}{result.m_HC2000[i]}" if ~result.m_HC2000.isna()[i] else f"HC2000 {result.HC2000[i]:.0f}" for i in range(len(result))], mask=result.HC2000.isna()).filled(''), format=str, description='Identifier in [HC2000], Hillenbrand & Carpenter (2000) [2000ApJ...540..236H]')
...
result_mrt['RVel']            = MaskedColumn(result_df['RVel'], mask=result_df['RVel'].isna(), unit=u.km/u.s, description='Radial velocity adopted for analysis in this study. RVel_NIRSPAO supplemented by RVel_APOGEE where the former is not available')
result_mrt['e_RVel']          = MaskedColumn(result_df['e_RVel'], mask=result_df['e_RVel'].isna(), unit=u.km/u.s, description='Uncertainty of radial velocity adopted for analysis in this study. e_RVel_NIRSPAO supplemented by e_RVel_APOGEE where the former is not available')
...
result_mrt['pmDE']            = MaskedColumn(result_df['pmDE'], mask=result_df['pmDE'].isna(), unit=u.mas/u.yr, description='Proper motion in declination adopted for analysis in this study. pmDE_K19 supplemented by pmDE_DR3 where the former is not available')
...
result_mrt['RVel_NIRSPAO']    = MaskedColumn(result_df['RVel_NIRSPAO'], mask=result_df['RVel_NIRSPAO'].isna(), unit=u.km/u.s, description='Radial velocity derived from NIRSPAO observation in this study')
result_mrt['e_RVel_NIRSPAO']  = MaskedColumn(result_df['e_RVel_NIRSPAO'], mask=result_df['e_RVel_NIRSPAO'].isna(), unit=u.km/u.s, description='Uncertainty of radial velocity derived from NIRSPAO in this study')
...
result_mrt['Veil Param O33']  = MaskedColumn(result_df['Veil Param O33'], mask=result_df['Veil Param O33'].isna(), unit=u.dimensionless_unscaled, description='Veiling parameter in order 32. Defined as in Theissen et al. (2022) [2022ApJ...926..141T]')
result_mrt['Veil Param O35']  = MaskedColumn(result_df['Veil Param O35'], mask=result_df['Veil Param O35'].isna(), unit=u.dimensionless_unscaled, description='Veiling parameter in order 32. Defined as in Theissen et al. (2022) [2022ApJ...926..141T]')
...
result_mrt['RVel_T22']        = MaskedColumn(result_df['RVel_T22'], mask=result_df['RVel_T22'].isna(), unit=u.km/u.s, description='Radial velocity from Theissen et al. (2022) [2022ApJ...926..141T]')
result_mrt['e_RVel_T22']      = MaskedColumn(result_df['e_RVel_T22'], mask=result_df['e_RVel_T22'].isna(), unit=u.km/u.s, description='Uncertainty of radial velocity from Theissen et al. (2022) [2022ApJ...926..141T]')
...
for model_name, bibcode in zip(['MIST','BHAC15', 'Feiden', 'Palla'], bibcodes):
    result_mrt[f'M_{model_name}'] = MaskedColumn(result_df[f'M_{model_name}'], mask=result_df[f'M_{model_name}'].isna(), unit=u.solMass, description=f'Stellar mass based on {model_name} model [{bibcode}]')
    result_mrt[f'e_M_{model_name}'] = MaskedColumn(result_df[f'e_M_{model_name}'], mask=result_df[f'e_M_{model_name}'].isna(), unit=u.solMass, description=f'Uncertainty of stellar mass based on {model_name} model [{bibcode}]')

for key, precision in rounding.items():
    result_mrt[key].info.format = f'.{precision}f'
...
result_mrt.write(sys.stdout, format='ascii.mrt')

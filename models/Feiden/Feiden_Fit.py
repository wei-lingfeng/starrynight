...
def fit_mag(teff):
    ...
    model_name = 'Feiden'
    model_suffix = '_mag'
    model_path = f'{user_path}/ONC/starrynight/models/' + model_name + '/' + model_name + model_suffix + '_Model.csv'
    ...
    result['mass_'   + model_name + model_suffix] = func_2(teff[:, 0])
    result['mass_e_' + model_name + model_suffix] = (np.nanmax(points, axis=1) - np.nanmin(points, axis=1)) / 2

    # if truth = nan, then lo & hi = nan
    result['mass_e_' + model_name + model_suffix][np.where( np.isnan(result['mass_' + model_name + model_suffix]) )] = np.nan
    ...
    result['logg_'   + model_name + model_suffix] = func_2(teff[:, 0])
    result['logg_e_' + model_name + model_suffix] = (np.nanmax(points, axis=1) - np.nanmin(points, axis=1)) / 2
    
    # if truth = nan, then lo & hi = nan
    result['logg_e_' + model_name + model_suffix][np.where( np.isnan(result['logg_' + model_name + model_suffix]) )] = np.nan
    ...
    result['logL_'   + model_name + model_suffix] = func_2(teff[:, 0])
    result['logL_e_' + model_name + model_suffix] = (np.nanmax(points, axis=1) - np.nanmin(points, axis=1)) / 2

    # if truth = nan, then lo & hi = nan
    result['logL_e_' + model_name + model_suffix][np.where( np.isnan(result['logL_' + model_name + model_suffix]) )] = np.nan
